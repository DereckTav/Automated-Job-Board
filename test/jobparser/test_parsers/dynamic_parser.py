...
"""
            ...
            with open("test/jobparser/data/dynamic_test.html", "r", encoding="utf-8") as f:
                html_content = f.read()
            ...
            if alternative_visit_count == 1:
                filename = "test/jobparser/data/dynamic_alternative_test.html"
            else:
                filename = "test/jobparser/data/dynamic_test.html"
...
async def test_parse(test_server):
    factory = get_factory()
    result = await factory.create_js_parser().parse(config)
...
async def test_parse_new_info(test_server, reset_singleton):
    factory = get_factory()
    parser = factory.create_js_parser()
